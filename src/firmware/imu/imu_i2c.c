...
AccelerometerSensor IMU_ReadAccelerometer(void) {
	uint8_t data[6];
	I2C_ReadNRegisters(ACCELGYRO_DEVICE, ACCEL_XOUT_H, 6, data);
	
	AccelerometerSensor ac_data;
...
GyroscopeSensor IMU_ReadGyroscope(void) {
	uint8_t data[6];
	I2C_ReadNRegisters(ACCELGYRO_DEVICE, GYRO_XOUT_H, 6, data);
...
MagnetometerSensor IMU_ReadMagnetometer(void) {
	...
	uint8_t data[7];
	I2C_ReadNRegisters(MAGNET_DEVICE, HXL, 7, data);
...
TermometerSensor IMU_ReadTermometer(void) {
	...
	term_data.temperature = (I2C_ReadRegister(ACCELGYRO_DEVICE, TEMP_OUT_H) << 8) + I2C_ReadRegister(ACCELGYRO_DEVICE, TEMP_OUT_L);
